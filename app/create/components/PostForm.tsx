...
export default function PostForm() {
  ...
  useEffect(() => {
    if(user?.id){
      setData({ ...data, creatorId: user?.id as string })
    }
  }, [user])
    ...
    (
      <div className=" flex w-full bg-slate-950 p-8 rounded-2xl justify-between items-start flex-row gap-5 ">
        ...
        <div className=" flex flex-col w-full gap-4">
          ...
          {<div className={`${data.location.lat?"":" hidden"}`} style={{ height: "180px", width:"100%", paddingTop:"10px" }}>
          {typeof window!=="undefined"&&data.location.lat&&<MapContainer style={{width:"100%", height:"180px", borderRadius:"10px"}} center={[parseFloat(data.location.lat), parseFloat(data.location.lon)]} zoom={13} scrollWheelZoom={true} doubleClickZoom={true} dragging={true}>
  <TileLayer
    attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
    url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
  />
  <Marker position={[parseFloat(data.location.lat), parseFloat(data.location.lon)]}>
    <Popup>
